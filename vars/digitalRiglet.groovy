...
def    total=jsonObja.Bamboo.individualtotal[j].totalBuilds
 ...
 if(email==email1)
  ...
  def fcnt =jsonObja.JENKINS.individualfailure[j].Failure_cnt
	    def    total=jsonObja.JENKINS.individualbuilds[j].Total_cnt
 ...
 if(email==email1)
  ...
  def gcnt =jsonObjg.GITHUB.individual_commit_Details[j].User_Commits_count
	       def email1=jsonObjg.GITHUB.individual_commit_Details[j].User_email
	       if(email==email1)
  {
    LIST1.add(["toolName":name,"metricName":"commits","value":gcnt])
  }
      }
